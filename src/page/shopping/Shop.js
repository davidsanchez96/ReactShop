...
import {ShopListAll, ShopListEdit} from "../../utils/actionTypes";
...
class Shop extends Component {
    ...
    _handleSave = () => {
        const {shopListReducer, dispatch, navigation} = this.props;
        dispatch({type: ShopListEdit});
        navigation.setParams({
            editable: !navigation.state.params.editable, count: shopListReducer.get('cart') ?
                shopListReducer.get('cart').size : 0
        });
        ...
        if (editable) {
            return (
                <View style={[styles.settlement, styles.editBar]}>
                    {/*左侧全选*/}
                    <TouchableOpacity
                        onPress={() => this._handleCheck(dispatch)}
            ...
            return (
                <View style={styles.settlement}>
                    ...
                    <TouchableOpacity
                        ...
                        activeOpacity={0.8}
                        onPress={() => this._handleSubmit(checkedList,cart)}>
    ...
    _handleCheck(dispatch) {
        dispatch({type: ShopListAll});
    }


    _handleSubmit(checkedList,cart) {
                        ...
                        if (checkedList.has(p.get("shoppingCartId"))) {
                            if (m.get("codexType") == "11") {
                                ...
                                if (limitNumber < p.get("goodsNum")) {
                                    Toast.show('购买数量不能大于最大限购数量' + limitNumber);
                                    isCheck = true;
                                    return;
                                }
                            }
