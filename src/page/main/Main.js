...
import {shopListCount} from "../../action/shopListActions";
...
class Main extends Component {
    ...
    _handleScroll = (e) => {
        ...
        if (offsetY >= 10) {
            ...
            this.props.dispatch(change(`rgba(230,58,89,${floatNum / 10})`));
        } else if (offsetY <= 5) {
            this.props.dispatch(change('rgba(0,0,0,.1)'));
        }


        console.log(offsetY);
        this.props.dispatch(show(offsetY >= -5));
        // msg.emit('index:showTopbar', offsetY >= -5 && !this._swipeMode);
    }
